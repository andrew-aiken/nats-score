...
func NewAuthMiddleware(natsAuthService *nats.NATSAuthService, requiredRoles map[string]string) *AuthMiddleware {
...
func (m *AuthMiddleware) authenticate(r *http.Request) (AuthResult, *nats.UserClaims) {
...
func GetClaimsFromContext(ctx context.Context) *nats.UserClaims {
	if claims, ok := ctx.Value(ClaimsContextKey).(*nats.UserClaims); ok {
		return claims
	}
	return nil
}
